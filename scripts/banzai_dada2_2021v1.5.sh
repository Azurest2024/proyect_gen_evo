...
SCRIPT_DIR="/home/gsotil/banzai_test/Pipeline_scripts/"
...
mkdir -p "${ANALYSIS_DIR}/opt/blast/Worker_Output"
...
mkdir -p "$FinalDestination" > /dev/null 2>&1
...
if [ "$(ls -A "${FinalDestination}")" ]; then
	...
	#echo RSCRIPTS="/home/gsotil/banzai_test/Pipeline_scripts/" >> "$POST_BLAST_FILE"
	#
	...
	echo "###################################################################################################" >> "$POST_BLAST_FILE"
	cat /home/gsotil/banzai_test/Pipeline_scripts/Post_BLAST_processing_dada2_v2.0.sh  >> "$POST_BLAST_FILE"
else
	echo
